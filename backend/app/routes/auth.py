...
def registro():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": "Tipo de usuário inválido. Use: cliente, gerente ou mecanico"
                }
            ),
            400,
        )
    ...
    # Criar usuário
    usuario = Usuario(nome=data["nome"], email=data["email"], tipo=tipo)
...
def perfil():
    ...
    @token_required
    def get_perfil():
        usuario = db.session.get(Usuario, request.usuario_id)
        if not usuario:
