...
class TestValidacaoRegistro:
    ...
    @pytest.mark.parametrize("nome,email,senha,tipo,expected_status,expected_message", [
        # Casos de sucesso
        ("João Silva", "joao@email.com", "senha123", "cliente", 201, "Usuário registrado com sucesso"),
        ("Maria Santos", "maria@email.com", "senha456", "mecanico", 201, "Usuário registrado com sucesso"),
        ("Admin User", "admin@email.com", "admin123", "gerente", 201, "Usuário registrado com sucesso"),
        
        # Casos de erro - tipo inválido
        ("Nome Teste", "tipo@email.com", "senha123", "invalido", 400, "Tipo de usuário inválido"),
        ("Nome Teste", "admin2@email.com", "senha123", "supervisor", 400, "Tipo de usuário inválido"),
    ])
    def test_validacao_registro(self, client, nome, email, senha, tipo, expected_status, expected_message):
    ...
    @pytest.mark.parametrize("payload,campo_ausente", [
        ({'email': 'test@email.com', 'senha': 'senha123', 'tipo': 'cliente'}, 'nome'),
    ...
    def test_registro_campo_obrigatorio(self, client, payload, campo_ausente):
        ...
        if 'email' in payload:
            ...
            payload['email'] = payload['email'].replace("@", f"{random.randint(1000,9999)}@")
...
class TestValidacaoLogin:
    ...
    @pytest.mark.parametrize("email,senha,expected_status", [
        # Login correto
    ...
    def test_validacao_login(self, client, usuario_cadastrado, email, senha, expected_status):
...
class TestValidacaoVeiculo:
    ...
    @pytest.mark.parametrize("placa,modelo,marca,ano,expected_status", [
        # Casos de sucesso
        ("ABC1234", "Civic", "Honda", 2020, 201),
        ("DEF5678", "Corolla", "Toyota", 2019, 201),
        ("GHI9012", "Gol", "Volkswagen", 2018, 201),
    ])
    def test_validacao_veiculo(self, client, auth_token, placa, modelo, marca, ano, expected_status):
...
class TestValidacaoServico:
    ...
    @pytest.mark.parametrize("descricao,expected_status", [
        # Casos de sucesso
        ("Troca de óleo", 201),
    ...
    def test_validacao_servico(self, client, setup_servico, descricao, expected_status):
        ...
        response = client.post('/api/servicos',
            ...
            json={
                'descricao': descricao,
                'veiculo_id': setup_servico['veiculo_id']
            }
        )
...
class TestValidacaoStatusServico:
    ...
    @pytest.mark.parametrize("novo_status,expected_status", [
        # Transições válidas
        ("em_andamento", 200),
        ("concluido", 200),
        ("cancelado", 200),
    ...
    def test_transicao_status(self, client, servico_setup, novo_status, expected_status):
        """Testa diferentes transições de status de serviço"""
        response = client.put(f'/api/servicos/{servico_setup["servico_id"]}',
...
class TestFormatacaoPlaca:
    ...
    @pytest.mark.parametrize("placa,valida", [
...
class TestCalculoValorServico:
    ...
    @pytest.mark.parametrize("orcamentos,expected_total", [
        # Sem orçamentos
        ([], 0.0),
...
class TestDashboardAPI:
    ...
    @pytest.mark.parametrize("tipo_usuario,email_key", [
        ('gerente', 'gerente_email'),
        ('mecanico', 'mecanico_email'),
        ('cliente', 'cliente_email'),
    ])
    def test_dashboard_api_por_tipo(self, client, setup_dashboard, tipo_usuario, email_key):
        """Testa que dashboard API retorna dados para cada tipo de usuário"""
        response = client.post('/auth/login', json={
            'email': setup_dashboard[email_key],
            'senha': 'senha123'
        })
        token = response.get_json()['token']
...
class TestUsuariosAPI:
    ...
    @pytest.mark.parametrize("rota,metodo,expected_status", [
        ('/api/usuarios', 'GET', 200),
    ])
    def test_usuarios_api_gerente(self, client, gerente_token, rota, metodo, expected_status):
...
class TestServicosAPICRUD:
    ...
    @pytest.mark.parametrize("status_novo", [
        'pendente',
        'em_andamento', 
    ...
    def test_atualizar_status_servico(self, client, setup_servicos_crud, status_novo):
        ...
        response = client.post('/auth/login', json={
            'email': setup_servicos_crud['gerente_email'],
            'senha': 'senha123'
        })
        token = response.get_json()['token']
        
        response = client.put(f'/api/servicos/{setup_servicos_crud["servico_id"]}',
            headers={'Authorization': f'Bearer {token}'},
