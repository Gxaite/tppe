...
def test_criar_veiculo_cliente(client, app, auth_headers_cliente, usuario_cliente):
    ...
    response = client.post(
        ...
        json={"placa": "ABC1234", "modelo": "Civic", "marca": "Honda", "ano": 2020},
    )
...
def test_atualizar_veiculo_proprio(client, app, auth_headers_cliente, usuario_cliente):
    ...
    response = client.put(
        f"/api/veiculos/{veiculo_id}", headers=auth_headers_cliente, json={"ano": 2019}
    )
    assert response.status_code == 200
    data = response.get_json()
...
def test_deletar_veiculo_proprio(client, app, auth_headers_cliente, usuario_cliente):
    ...
    response = client.delete(
        f"/api/veiculos/{veiculo_id}", headers=auth_headers_cliente
    )
    assert response.status_code == 200


def test_criar_veiculo_placa_duplicada(
    client, app, auth_headers_cliente, usuario_cliente
):
    ...
    response = client.post(
        "/api/veiculos",
        headers=auth_headers_cliente,
        json={"placa": "JKL3456", "modelo": "Outro", "marca": "Outra", "ano": 2020},
    )
