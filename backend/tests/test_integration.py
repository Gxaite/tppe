...
def usuarios(app):
    ...
    with app.app_context():
        ...
        mecanico = Usuario(
            nome="Mecânico Teste", email="mecanico@test.com", tipo=TipoUsuario.MECANICO
        )
        ...
        return {"gerente": gerente, "mecanico": mecanico, "cliente": cliente}
...
class TestFluxoAutenticacao:
    ...
    def test_fluxo_registro_e_login(self, client):
        ...
        response = client.post(
            "/login",
            data={"email": "novo@cliente.com", "senha": "senha123"},
            follow_redirects=True,
        )
    ...
    def test_fluxo_login_invalido(self, client, usuarios):
        ...
        # Senha incorreta
        response = client.post(
            "/login", data={"email": "cliente@test.com", "senha": "senha_errada"}
        )
        ...
        response = client.post(
            "/login", data={"email": "naoexiste@test.com", "senha": "senha123"}
        )
    ...
    def test_logout_limpa_sessao(self, client, usuarios):
        ...
        # Fazer login
        client.post("/login", data={"email": "cliente@test.com", "senha": "senha123"})
...
class TestFluxoServicos:
    ...
    def login_mecanico(self, client):
        ...
        client.post("/login", data={"email": "mecanico@test.com", "senha": "senha123"})
    ...
    def test_fluxo_completo_servico(self, client, usuarios, veiculo, app):
        ...
        response = client.post(
            f"/orcamentos/novo?servico_id={servico_id}",
            data={
        ...
        response = client.post(
            f"/orcamentos/{orcamento_id}/aprovar", follow_redirects=True
        )
        assert response.status_code == 200
...
class TestFluxoDashboards:
    ...
    def test_dashboard_cliente_mostra_dados_corretos(
        self, client, usuarios, veiculo, app
    ):
    ...
    def test_dashboard_mecanico_mostra_servicos_atribuidos(
        self, client, usuarios, veiculo, app
    ):
    ...
    def test_dashboard_gerente_mostra_visao_geral(self, client, usuarios, veiculo, app):
        ...
        # Login como gerente
        client.post("/login", data={"email": "gerente@test.com", "senha": "senha123"})
